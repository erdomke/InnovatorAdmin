...
namespace RoslynPad
{
  public partial class DocumentView : IDisposable
  {
    ...
    private RegionFoldingStrategy _foldingStrategy = new RegionFoldingStrategy();
    ...
    private void EditorOnPreviewMouseWheel(object sender, MouseWheelEventArgs args)
    {
      ...
      {
        _viewModel.ParentViewModel.EditorFontSize += args.Delta > 0 ? 1 : -1;
        args.Handled = true;
      }
    }
    ...
    {
      _viewModel = args.NewValue as MethodViewModel;
      if (_viewModel == null)
        return;

      _viewModel.NuGet.PackageInstalled += NuGetOnPackageInstalled;
      _roslynHost = _viewModel.ParentViewModel.RoslynHost;

      _viewModel.ParentViewModel.EditorFontSizeChanged += OnEditorFontSizeChanged;
      Editor.FontSize = _viewModel.ParentViewModel.EditorFontSize;
    ...
    private readonly HashSet<string> _warningsToIgnore = new HashSet<string>(new string[] { "CS1701" });

    private void ProcessDiagnostics(DiagnosticsUpdatedArgs args)
    {
      _textMarkerService.RemoveAll(x => true);

      foreach (var diagnosticData in args.Diagnostics.Where(d => !_warningsToIgnore.Contains(d.Id)))
      {
    ...
    public void Dispose()
    {
      if (_viewModel?.ParentViewModel != null)
      {
        _viewModel.ParentViewModel.EditorFontSizeChanged -= OnEditorFontSizeChanged;
      }
    }
