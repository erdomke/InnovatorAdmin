...
namespace ADiff
{
  ...
  public class DiffMatchPatch {
    ...
    public List<Diff> diff_main_line(string text1, string text2, DateTime deadline)
    {
    ...
    public struct ThreeWayDiff
    {
      public int Parent;
      public int Left;
      public int Right;

      public ThreeWayDiff(int parent, int left, int right)
      {
        this.Parent = parent;
        this.Left = left;
        this.Right = right;
      ...
      public override string ToString()
      {
        return string.Format("{0:D3} <- {1:D3} -> {2:D3}", Left, Parent, Right);
      }
    }

    public struct ThreeWayString
    {
      private int _exists;

      public string Value;

      public ThreeWayString(string value, bool parent, bool left, bool right)
      {
        this.Value = value;
        this._exists = (left ? 4 : 0) + (parent ? 2 : 0) + (right ? 1 : 0);
      }
    ...
    ///// <remarks>Based on paper by Paul Heckel at http://documents.scribd.com/docs/10ro9oowpo1h81pgh1as.pdf </remarks>
    //public static void MoveDiff(string oldValue, string newValue)
    //{
    ...
    //  string charsOld = oldValue; //diff_linesToCharsMunge(oldValue, lineArray, lineHash);
    //  string charsNew = newValue; //diff_linesToCharsMunge(newValue, lineArray, lineHash);

    //  var symbolTable = new Dictionary<int, SymbolData>();
    //  var oldSymbols = new List<SymbolData>();
    //  var newSymbols = new List<SymbolData>();
    //  SymbolData symbolEntry;

    //  var begin = SymbolData.BeginSymbol();
    //  var end = SymbolData.EndSymbol();
    //  symbolTable.Add(begin.Symbol, begin);
    //  oldSymbols.Add(begin);
    //  newSymbols.Add(begin);
    //  symbolTable.Add(end.Symbol, end);

    //  foreach (var ch in charsNew)
    //  {
    //    if (!symbolTable.TryGetValue((int)ch, out symbolEntry))
    //    {
    //      symbolEntry = new SymbolData() { Symbol = (int)ch };
    //      symbolTable.Add(symbolEntry.Symbol, symbolEntry);
    //    }
    //    symbolEntry.IncrementNewCount();
    //    newSymbols.Add(symbolEntry);
    //  }
    //  for (var i = 0; i < charsOld.Length; i++)
    //  {
    //    if (!symbolTable.TryGetValue((int)charsOld[i], out symbolEntry))
    //    {
    //      symbolEntry = new SymbolData() { Symbol = (int)charsOld[i] };
    //      symbolTable.Add(symbolEntry.Symbol, symbolEntry);
    //    }
    //    symbolEntry.IncrementOldCount();
    //    symbolEntry.OldLineNumber = i + 1;
    //    oldSymbols.Add(symbolEntry);
    //  }

    //  end.OldLineNumber = charsOld.Length + 1;
    //  oldSymbols.Add(end);
    //  newSymbols.Add(end);

    //  var oldNums = new int[oldSymbols.Count];
    //  var newNums = new int[newSymbols.Count];
    ...
    //  {
    //    if (newSymbols[i].IsAnchor)
    //    {
    //      newNums[i] = newSymbols[i].OldLineNumber;
    //      oldNums[newNums[i]] = i;
    //    }
    //    else
    //    {
    ...
    //      oldNums[newNums[i + 1]] = i + 1;
    //    }
    //  }

    //  for (var i = newNums.Length - 1; i > 0; i--)
    //  {
    //    if (newNums[i] >= 0
    //      && newNums[i] > 0
    //      && newNums[i - 1] < 0
    ...
    //  public bool IsAnchor { get { return _counts == 0x5; } }
    //  public SymbolCount NewCount
    //  {
    ...
    //  public void IncrementOldCount()
    //  {
    //    _counts += 1;
    //    if ((_counts & 0x3) == 0x3) _counts &= 0xe;
    //  }
    //  public void IncrementNewCount()
    //  {
    //    _counts += 4;
    //    if ((_counts & 0xc) == 0xc) _counts &= 0xb;
    //  }

    //  public static SymbolData BeginSymbol()
    //  {
    //    return new SymbolData() { Symbol = -1, _counts = 0x5, OldLineNumber = 0 };
    //  }
    //  public static SymbolData EndSymbol()
    //  {
    //    return new SymbolData() { Symbol = -2, _counts = 0x5 };
    //  }
    //}
    ...
    //public IEnumerable<ThreeWayString> diff_three_way(string parent, string text1, string text2)
    //{
    //  return diff_three_way(parent, text1, text2, DateTime.Now.AddMilliseconds(5000));
    //}
    //public IEnumerable<ThreeWayString> diff_three_way(string parent, string text1, string text2, DateTime deadline)
    //{
    //  List<string> lineArray;
    //  var diffs = diff_three_way(parent, text1, text2, deadline, out lineArray);
    //  var result = new List<ThreeWayString>();
    //  foreach (var diff in diffs)
    //  {
    //    result.Add(new ThreeWayString(lineArray[Math.Max(Math.Max(diff.Left, diff.Parent), diff.Right)],
    //      diff.Parent >= 0, diff.Left >= 0, diff.Right >= 0));
    //  }
    //  return result;
    //}

    //private enum MergeStats
    //{
    //  None = 0,
    //  RightOnly = 1,
    //  ParentOnly = 2,
    ...
    public MergeDocument diff_three_way(string parent, string text1, string text2)
    {
      ...
      Action processBuilders = () => {
        ...
        var found = false;
        foreach (var alt in block.Alternates)
        {
          if (alt.Text == parentBuilder.ToString())
          {
            alt.Location |= MergeLocation.Parent;
            found = true;
            break;
          }
        }
        ...
        if (block.Alternates.Count == 1)
        {
          block.Alternates[0].Location |= MergeLocation.Output;
          block.IsConflict = false;
        }
        else if (block.Alternates.Count == 2)
        {
          if (block.Alternates.Any(b => b.Location == MergeLocation.Parent))
          {
      ...
      foreach (var diff in diffs)
      {
        ...
        if (diff.Left >= 0)
        {
          leftBuilder.Append(text);
          leftCount++;
        }
        if (diff.Right >= 0)
        {
          rightBuilder.Append(text);
          rightCount++;
        }
        if (diff.Parent >= 0)
        {
          parentBuilder.Append(text);
          parentCount++;
        }

        lastAllEqual = currAllEqual;
      }
      processBuilders();
    ...
    //private MergeStats DiffStats(ThreeWayDiff diff)
    //{
    //  return (MergeStats)((diff.Left < 0 ? 0 : 4) + (diff.Parent < 0 ? 0 : 2) + (diff.Right < 0 ? 0 : 1));
    //}

    private IEnumerable<ThreeWayDiff> diff_three_way(string parent, string text1, string text2, DateTime deadline, out List<string> lineArray)
    {
      // Check for null inputs
      ...
      // So we'll insert a junk entry to avoid generating a null character.
      lineArray.Add(string.Empty);
      string charsParent = diff_linesToCharsMunge(parent, lineArray, lineHash);
      string chars1 = diff_linesToCharsMunge(text1, lineArray, lineHash);
      string chars2 = diff_linesToCharsMunge(text2, lineArray, lineHash);

      // Get the diffs from each side to the parent
      var diffs1 = diff_main(charsParent, chars1, false, deadline);
      diff_cleanupSemantic(diffs1, false);
      diff_cleanupSemanticLossless_Lines(diffs1, lineArray);
      var diffs2 = diff_main(charsParent, chars2, false, deadline);
      diff_cleanupSemantic(diffs2, false);
      diff_cleanupSemanticLossless_Lines(diffs2, lineArray);

      // Load the left side diffs
      IList<ThreeWayDiff> diffs = new List<ThreeWayDiff>();
        ...
        switch (diff.operation)
        {
          case Operation.DELETE:
            foreach (var ch in diff.text)
              diffs.Add(new ThreeWayDiff((int)ch, -1, -1));
            break;
          case Operation.EQUAL:
            foreach (var ch in diff.text)
              diffs.Add(new ThreeWayDiff((int)ch, (int)ch, -1));
            break;
          case Operation.INSERT:
            foreach (var ch in diff.text)
              diffs.Add(new ThreeWayDiff(-1, (int)ch, -1));
            break;
        }
      }

      // Align the right-side diffs
      var diffPtr = 0;
      Func<int, int> incrementDiffPtr = (offset) =>
      {
        var result = diffPtr;
        diffPtr += offset;
        while (diffPtr < diffs.Count && diffs[diffPtr].Parent < 0)
          diffPtr++;
        return result;
      };
      ...
      foreach (var diff in diffs2)
      {
        switch (diff.operation)
        {
          ...
          case Operation.EQUAL:
            ...
            {
              incrementDiffPtr(0);
              var mutate = diffs[diffPtr];
              mutate.Right = (int)ch;
              diffs[diffPtr++] = mutate;
            }
            break;
          case Operation.INSERT:
            foreach (var ch in diff.text)
              diffs.Insert(diffPtr++, new ThreeWayDiff(-1, -1, (int)ch));
            break;
        }
      }

      // Do a comparison of non-matching left and right blocks to align them as appropriate
      diffs = AlignOrphanLeftRightDiffs(diffs, lineArray);

      return diffs;
    }

    private IList<ThreeWayDiff> AlignOrphanLeftRightDiffs(IList<ThreeWayDiff> diffs, List<string> lineArray)
    {
      var lastCopy = -1;
      bool containsLeftOnly = false;
      bool containsRightOnly = false;
      var result = new List<ThreeWayDiff>();
      var leftBuilder = new StringBuilder();
      var rightBuilder = new StringBuilder();
      List<Diff> leftRightDiffs;
      int i = 0;

      Action compareLeftRight = () =>
      {
        if (lastCopy < i - 1)
        {
          if (containsLeftOnly && containsRightOnly)
          {
            leftBuilder.Length = 0;
            rightBuilder.Length = 0;

            for (var j = lastCopy + 1; j < i; j++)
            {
              if (diffs[j].Left >= 0)
              {
                leftBuilder.Append((char)diffs[j].Left);
              }
              else if (diffs[j].Right >= 0)
              {
                rightBuilder.Append((char)diffs[j].Right);
              }
              else
              {
                result.Add(diffs[j]);
            ...
            leftRightDiffs = diff_main(leftBuilder.ToString(), rightBuilder.ToString(), false);
            diff_cleanupSemantic(leftRightDiffs, false);
            diff_cleanupSemanticLossless_Lines(leftRightDiffs, lineArray);

            foreach (var diff in leftRightDiffs)
            {
              switch (diff.operation)
              {
                case Operation.DELETE:
                  foreach (var ch in diff.text)
                    result.Add(new ThreeWayDiff(-1, (int)ch, -1));
                  break;
                case Operation.EQUAL:
                  foreach (var ch in diff.text)
                    result.Add(new ThreeWayDiff(-1, (int)ch, (int)ch));
                  break;
                case Operation.INSERT:
                  foreach (var ch in diff.text)
                    result.Add(new ThreeWayDiff(-1, -1, (int)ch));
                  break;
              }
            }
          }
      ...
      {
        if (diffs[i].Left < 0 && diffs[i].Right < 0)
        {
          // Do nothing
        }
        else if (diffs[i].Left < 0 && diffs[i].Parent < 0)
        {
          containsRightOnly = true;
        }
        else if (diffs[i].Right < 0 && diffs[i].Parent < 0)
        {
          containsLeftOnly = true;
        }
        else
        {
          compareLeftRight();
     ...
     * @param diffs List of Diff objects.
     */
    public void diff_cleanupSemantic(List<Diff> diffs, bool performLossless = true) {
      bool changes = false;
      ...
      if (performLossless) diff_cleanupSemanticLossless(diffs);
    ...
    public void diff_cleanupSemanticLossless_Lines(List<Diff> diffs, List<string> lines)
    {
      int pointer = 1;
    ...
    private Regex LinePattern_SgmlEndTag = new Regex(@"^\s*</\w+>");

    private int diff_cleanupSemanticScore_Line(string one, string two)
    {
      ...
      var blankLine1 = string.IsNullOrEmpty(one.Trim());
      var blankLine2 = string.IsNullOrEmpty(two.Trim());
      // XML, HTML
      var isXmlEnd1 = LinePattern_SgmlEndTag.IsMatch(one);
      var oneTrimmed = one.TrimStart();
      // VB
      var isEndKeyword1 = oneTrimmed.StartsWith("end", StringComparison.OrdinalIgnoreCase);
      // C#, Java, Javascript, ...
      var isCloseBrace1 = !string.IsNullOrEmpty(oneTrimmed) &&
        (oneTrimmed[0] == '}' || oneTrimmed[0] == ')' || oneTrimmed[0] == ']');
      // SQL
      var isSemicolon1 = !string.IsNullOrEmpty(oneTrimmed) && oneTrimmed[0] == ';';
      ...
      if (isXmlEnd1 || isEndKeyword1 || isCloseBrace1)
      {
        return 4;
      }
      if (isSemicolon1)
