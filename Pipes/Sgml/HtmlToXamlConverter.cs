...
namespace WpfRichText.XamlToHtmlParser
{
  using System;
  using System.Xml;
  ...
  public static class HtmlToXamlConverter
  {
    ...
    /// </param>
    private static int GetSpan(XmlElement htmlTDElement, bool row)
    {
      string spanAsString;
      int span;

      spanAsString = GetAttribute((XmlElement)htmlTDElement, (row ? "rowspan" : "colspan"));
      if (spanAsString != null)
      {
        if (!Int32.TryParse(spanAsString, out span))
        {
