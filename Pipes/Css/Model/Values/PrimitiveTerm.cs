...
namespace Pipes.Css
{
  public class PrimitiveTerm : Term
  {
    ...
    public static PrimitiveTerm FromNumberString(string value)
    {
      if (string.IsNullOrEmpty(value)) return null;
      int i = 0;
      while (i < value.Length && (char.IsDigit(value[i]) || value[i] == '.')) i++;
      if (i < 1) throw new ArgumentException("No number found");
      var sng = Single.Parse(value.Substring(0, i));
      if (i >= value.Length) return new PrimitiveTerm(UnitType.Pixel, sng);
      return new PrimitiveTerm(value.Substring(i).Trim(), sng);
    }
