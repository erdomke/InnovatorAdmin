...
namespace InnovatorAdmin.Editor
{
  ...
  public class XmlSchemaCompletionData
  {
    ...
    /// <summary>
    /// Creates completion data from a schema object.
    /// </summary>
    public XmlSchemaCompletionData(XmlSchema schema)
    {
      ReadSchema(schema);
    }
    ...
    void ReadSchema(XmlReader reader)
    {
      try
      {
        ReadSchema(XmlSchema.Read(reader, new ValidationEventHandler(SchemaValidation)));
      }
      finally
      {
        reader.Close();
    ...
    XmlSchemaAttribute FindAttribute(XmlSchemaComplexType complexType, string name)
    {
      ...
      if (matchedAttribute == null)
      {
        ...
        {
          matchedAttribute = FindAttribute(simpleContent, name);
        }
      }
