...
namespace RoslynPad
{
  public partial class MainWindow
  {
    ...
    public MainWindow() : this(SynchronizationContext.Current) { }

    public MainWindow(SynchronizationContext syncContext)
    {
      InnovatorAdmin.Program.EnsureApplicationResources();
      ...
      var refAssemblies = new List<AssemblyRef>()
            {
                AssemblyRef.FromType(typeof(object)),
                AssemblyRef.FromType(typeof(System.Threading.Tasks.Task)),
                AssemblyRef.FromType(typeof(List<>)),
                AssemblyRef.FromType(typeof(System.Text.RegularExpressions.Regex)),
                AssemblyRef.FromType(typeof(System.Text.StringBuilder)),
                AssemblyRef.FromType(typeof(Uri)),
                AssemblyRef.FromType(typeof(Enumerable)),
                AssemblyRef.FromType(typeof(ObjectExtensions))
                , AssemblyRef.FromType(typeof(System.Xml.XmlNode))
                , new AssemblyRef { Location = "IOM.dll", Namespaces = new [] { "Aras.IOM" } }
                , new AssemblyRef { Location = "InnovatorCore.dll", Namespaces = new [] { "Aras.Server.Core" } }
            };

      _roslynHost = new RoslynHost(refAssemblies);
    ...
    private void ProcessDiagnostics(DiagnosticsUpdatedArgs args)
    {
      ...
      foreach (var diagnosticData in args.Diagnostics.Where(d => d.Id != "CS1701"))
      {
