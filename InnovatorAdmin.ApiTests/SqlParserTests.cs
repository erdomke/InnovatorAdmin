...
END";
    ...
    public void FindSqlServerObjectNamesTest()
    {
      ...
      var names = DependencyAnalyzer.GetInnovatorNames(SampleSql)
        .Select(n => n.FullName).ToArray();
      CollectionAssert.AreEqual(correct, names);
      ...
      names = DependencyAnalyzer.GetInnovatorNames(SampleSql_PartRollup)
        .Select(n => n.FullName).ToArray();
      CollectionAssert.AreEqual(correct, names);
    }
    ...
    private const string SampleSql = @"SELECT
    c.[id]
...
CROSS APPLY innovator.ConvertToLocalTbl(com.event_date, 'Eastern Standard Time') d_ev";
    private const string SampleSql_PartRollup = @"/*
